...
,CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', 
    CONCAT(
        COALESCE(b.SegmentKey, ''), '|', 
        COALESCE(c.CountryKey, '')
    )), 2) AS key_geo
,CONVERT(VARCHAR(64), HASHBYTES('SHA2_256', 
    CONCAT(
        ...
        COALESCE(CASE WHEN MAX(BetSelectionTypeKey) > 1 THEN 2 ELSE 1 END, ''), '|',
...
,CASE WHEN numberofselections=1 AND CouponTypeKey=1 
	THEN BetGroupKey ELSE 0 END AS BetGroupKey						-- KEY to DW.BetGroup
